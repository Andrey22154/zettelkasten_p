...
def get_similar_notes(note: str):
    ...
    for note in dataset:
        ...
        if common_tags:
            ...
            # Вычисление косинусного сходства между входной заметкой и заметкой из датасета
            similarity = cosine_similarity(input_embeddings, note_embeddings)[0][0]

            # Если сходство больше некоторого порога, добавляем заметку в список похожих
            if similarity > 0.99:
